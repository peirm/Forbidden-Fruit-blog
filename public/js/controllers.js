...
//bodyCtrl控制器
    ...
    .controller('mainArticlesCtrl',function ($scope,$http,moment) {
        ...
        $scope.likeArticle = function (article) {
            ...
            let isliked = article.likeUserIds.indexOf($scope.user._id);
            if(isliked == '-1') {
                $http({
                    method:"GET",
...
//用户注册页面userRegApp
const userRegApp = angular.module('userRegApp',[]);
//用户注册页面注册模块
...
//用户更换头像
    ...
    .controller('userArticlesCtrl',function ($scope,$http,$location) {
        ...
        $scope.collectArticle = function (article) {
            ...
            let isCollected = article.collectUserIds.indexOf($scope.user._id);
            if(isCollected == '-1') {
                if(article.author === $scope.user.username) {
                    return alert('不能收藏自己的文章哦');
                }
...
//日志发表页面diaryPostApp
const diaryPostApp = angular.module('diaryPostApp',[]);
//日志发布页面日志发布模块
...
//用户信息页面editInfoApp
const editInfoApp = angular.module('editInfoApp',[]);
//用户信息页面用户信息修改模块
...
//文章修改页面editDiaryApp
const editDiaryApp = angular.module('editDiaryApp',[]);
...
const articleInfoApp = angular.module('articleInfoApp',[]);
//主要内容控制器
